...
- `.claude/skills/**` is not, and is restored from `skills-lock.json` the way
  dependencies are restored from a lockfile. A subagent that reads a skill must
  say so and stay usable when the file is absent, as `security-reviewer` does.
